...
#
# CLOUDFLARE PAGES APPLIES EVERY MATCHING RULE AND CONCATENATES THE VALUES. It does
# not pick the most specific one. An earlier version of this file had /assets/css/*,
# /assets/* and /* all setting Cache-Control, so a stylesheet came back as
#
#   cache-control: public, max-age=31536000, immutable, public, max-age=31536000,
#                  immutable, public, max-age=0, must-revalidate
#
# which is three rules stacked and ends on must-revalidate. Observed live, not
# theorised. So: no two rules below may set Cache-Control on the same path, and the
# catch-all sets security headers ONLY.

# Everything under /assets is either content-hashed (the extracted CSS) or carries
# ?v=ASSET_VERSION (images). Either way a change is a new URL, so it can be cached
# forever. One rule, covering css, js and images together, because /assets/css/* and
# /assets/* would both match a stylesheet.
/assets/*
  Cache-Control: public, max-age=31536000, immutable

# The scheduling bundle is served from /js/, outside /assets, and its filename is not
# hashed — so it has to revalidate or a wizard fix would never reach anyone.
/js/*
  Cache-Control: public, max-age=0, must-revalidate

# Security headers for everything. Deliberately NO Cache-Control here: it would stack
# onto both rules above. HTML gets Cloudflare's default, which is already
# public, max-age=0, must-revalidate — the right answer for pages carrying prices and
# phone numbers.
/*
  X-Content-Type-Options: nosniff
  Referrer-Policy: strict-origin-when-cross-origin{preview_noindex}
"""
