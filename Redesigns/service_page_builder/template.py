...
CDN = "https://cdn.jsdelivr.net/gh/extremheating-cloud/extreme-assets@main/images/brand"
# Content photography lives alongside the brand assets in the same repo.
CDN_DESCRIPTIVE = "https://cdn.jsdelivr.net/gh/extremheating-cloud/extreme-assets@main/images/descriptive"

ASSET_REPO = "https://cdn.jsdelivr.net/gh/extremheating-cloud/extreme-assets"

def cdn_asset(relpath, commit=None):
    """URL for anything under images/ in the asset repo, optionally commit-pinned.

    relpath is repo-relative below images/ — "descriptive/RUUD Heat Pump.jpg" or
    "About/Troy.jpg". Spaces are percent-encoded here so call sites can use the real
    filename; extension case must match the repo exactly (Mason - Office.JPG).
    See cdn_descriptive below for why pinning matters.
    """
    from urllib.parse import quote
    return f"{ASSET_REPO}@{commit or 'main'}/images/{quote(relpath)}"

def cdn_descriptive(filename, commit=None):
    """URL for a content photo, optionally pinned to a commit.

    Replacing a file in place does NOT change what jsDelivr serves. `@main` is a
    branch ref, and jsDelivr caches the branch-to-commit resolution for 12 hours
    (s-maxage=43200) on top of a 7-day browser cache. Measured 2026-07-31: after a
    re-upload, `@main` still returned the old bytes, a `?v=` query string returned
    the old bytes, and even the purge API reported "finished" while `@main` kept
    serving the old commit.

    Passing a commit is the only thing that takes effect immediately, and it makes
    the URL a permanent pointer to specific bytes. Swap the commit when the photo
    is replaced; `git log` the asset repo, or use the GitHub commits API:
      /repos/extremheating-cloud/extreme-assets/commits?path=images/descriptive/<file>
    """
    return f"{ASSET_REPO}@{commit or 'main'}/images/descriptive/{filename}"
X_MARK = f"{CDN}/x-mark.png"

# ---------------------------------------------------------------- photography
# Real Extreme photography from the asset repo, commit-pinned. jsDelivr caches the
# branch-to-commit resolution for 12 hours, so replacing a file in place does NOT
# change what the site serves — pinning makes each URL point at exact bytes.
# Replacing a photo means updating the commit here, not just re-uploading.
#
...
PHOTOS = {
    ...
    "ruudInstall":  cdn_asset("descriptive/ruud-install.jpg",               _C_RUUD_IN),
}
